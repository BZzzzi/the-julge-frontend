...
import type { SeoulRegion, ShopCategory, ShopItem } from "@/types/shop";
import Image from "next/image";
import { useRouter } from "next/navigation";
import { useRef, useState } from "react";

type Props = {
  // userRes.item.shop?.item 이 들어오는 타입이 ShopItem 이라 가정
  initialShop: ShopItem["item"];
};
...
type FormState = {
  ...
  address2: string;
  description: string;
  originalHourlyPay: string; // input은 string
};

type FormErrors = Partial<Record<keyof FormState | "imageUrl", string>>;

export default function ShopEditForm({ initialShop }: Props) {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    address2: initialShop.address2 ?? "",
    category: (initialShop.category as ShopCategory) ?? "",
    description: initialShop.description ?? "",
    originalHourlyPay: String(initialShop.originalHourlyPay ?? ""),
  });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "가게 수정 실패";
      setErrorMessage(msg);
      setErrorOpen(true);
    } finally {
      setSubmitting(false);
        ...
        <div className="mx-auto w-full max-w-241 rounded-2xl p-8">
          ...
          >
            ...
            <div className="flex justify-center pt-2">
              ...
              >
                {submitting ? "수정 중..." : uploading ? "업로드 중..." : "수정하기"}
              </Button>
            </div>
          </form>
        </div>
